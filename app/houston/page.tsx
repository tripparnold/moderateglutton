...
function FilterBar({ filters, onChange, showTop10 }: { filters: Filters; onChange: (f: Filters) => void; showTop10: boolean }) {
  ...
  const pillOff = `${pill} border-espresso/30 text-tan hover:border-tan hover:text-espresso`;
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {showTop10 && (
          <button onClick={() => set('top10Only', !filters.top10Only)}
            className={`${pill} ${filters.top10Only ? 'bg-lapis text-sand border-lapis' : 'border-espresso/30 text-tan hover:border-lapis hover:text-lapis'}`}>
            Tripp&apos;s Top 10
          </button>
        )}
        <button onClick={() => setOpen(!open)}
          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border transition-colors font-medium ${hasOtherFilters ? 'border-lapis text-lapis bg-lapis/5' : 'border-espresso/30 text-muted hover:border-tan hover:text-tan'}`}>
  ...
  const viewBtn = (active: boolean) =>
    `px-4 py-2 rounded-full text-sm font-medium transition-colors ${
      active
        ...
        : 'bg-transparent text-espresso border border-espresso/30 hover:border-lapis hover:text-lapis'
    }`;
